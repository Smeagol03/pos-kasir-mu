...
$classes = ($active ?? false)
            ? 'group flex items-center px-3 py-2 text-sm font-semibold rounded-lg bg-indigo-50 text-indigo-700 transition-all duration-200'
            : 'group flex items-center px-3 py-2 text-sm font-medium rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200';

$iconClasses = ($active ?? false)
                ? 'text-indigo-600 mr-3 flex-shrink-0 h-5 w-5'
    ...
    @if($icon)
        @switch($icon)
            ...
            @case('heroicon-o-shopping-cart')
                ...
                @break
            @case('heroicon-o-receipt-refund')
                <svg class="{{ $iconClasses }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
